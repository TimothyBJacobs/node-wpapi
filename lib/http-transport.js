...
function returnBody( wpreq, result ) {
	...
	if ( typeof body === 'object' && body ) {
		if ( result.headers['allow'] ) {
			Object.defineProperty( body, '_allow', {
				value: result.headers['allow'].split( ', ' ),
			} );
		}

		if ( result.headers['link'] ) {
			Object.defineProperty( body, '_headerLinks', {
				value: parseLinkHeader( result.headers['link'], { extended: true } ).map( function ( link ) {
					link.href = link.link;
					link.rel = link.rel[0];
